...
Inheritance
IAxisTickSettings extends ITickSettings.
IAxisTickSettings is not extended by any other symbol.
Properties
...
> **Note:** This class also inherits all settings, properties, methods, and events from ITickSettings (and its ancestors). Use `get_doc` or `get_core_reference` with the parent class name to see inherited members.
