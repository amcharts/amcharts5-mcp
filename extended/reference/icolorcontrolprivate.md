---
title: "IColorControlPrivate"
type: "interface"
source: "https://www.amcharts.com/docs/v5/reference/icolorcontrolprivate/"
scraped: "2026-03-15"
---

Inheritance

## Inheritance

Extends: IStockControlPrivate

> **Note:** This class also inherits all settings, properties, methods, and events from IStockControlPrivate (and its ancestors). Use `get_doc` or `get_core_reference` with the parent class name to see inherited members.

## Properties

- **color** (`Color`) — (no description)
- **dropdown** (`DropdownColors`) — (no description)
- **opacity** (`undefined | number`) — (no description)
