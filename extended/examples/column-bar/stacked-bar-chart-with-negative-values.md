...
We use negative numbers for the "male" series but set chart to display absolute numbers only via chart.getNumberFormatter().set("numberFormat", "#.#s"). Then we make sure our columns are placed on top of each other (though they don't overlap due to values being on opposites sides of 0) by setting clustered: false on the series.
Formatting numbers
Clustering in Column series
